module clm_time_manager
  ...
  integer  :: stop_date_ymd   ! Year, month and day of the simulation stop date in yyyymmdd format (e.g., 19960801)
  integer  :: stop_date_tod   ! Time of day (UTC) of the simulation stop date (seconds past 0Z)
  ...
  real(r8) :: curr_calday     ! Current calendary day
  ...
  public :: get_prev_date     ! Return date components at beginning of timestep
  public :: get_prev_time     ! Return time components at beginning of timestep
  public :: get_prev_calday   ! Return calendar day at beginning of timestep
  ...
  public :: is_last_step      ! Return true on last timestep of run
  !-----------------------------------------------------------------------
...
contains
 ...
 10 continue
  ...
  subroutine get_prev_time (days, seconds)
    !
    ! !DESCRIPTION:
    ! Return the time components at the beginning of the timestep
    !
    ! !ARGUMENTS:
    implicit none
    integer, intent(out) :: days    ! Number of whole days in time interval
  ...
  !-----------------------------------------------------------------------
  function get_curr_calday() result(calday)
    !
    ! !DESCRIPTION:
    ! Return calendar day at end of current timestep. Calendar day 1.0 = 0Z on Jan 1
    !
    ! !LOCAL VARIABLES:
  ...
  !-----------------------------------------------------------------------
  function get_prev_calday() result(calday)
    !
    ! !DESCRIPTION:
  ...
  logical function is_last_step()
    !
    ! !DESCRIPTION:
    ! Return true on last timestep of run
    !
    ! !LOCAL VARIABLES:
    integer :: yr               ! Year (1900, ...)
    integer :: mon              ! Month (1, ..., 12)

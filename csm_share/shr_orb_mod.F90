module shr_orb_mod
  ...
  !-----------------------------------------------------------------------
  ! !DESCRIPTION:
  ! Calculates zenith angle, solar declination, and orbital parameters
  !
  ! !USES:
  use shr_kind_mod, only : SHR_KIND_R8, r8 => shr_kind_r8
  ...
  public :: shr_orb_cosz       ! Cosine of the solar zenith angle
  public :: shr_orb_decl       ! Eccentricity factor and solar declination
  public :: shr_orb_params     ! Earth's orbital parameters
  !-----------------------------------------------------------------------
...
contains
  ...
  real(r8) function shr_orb_cosz (jday, lat, lon, declin)
    !
    ! !DESCRIPTION:
    ! Return the cosine of the solar zenith angle. Assumes 365.0 days per year
    !
    ! !ARGUMENTS:
    implicit none
    real(r8), intent(in) :: jday   ! Julian calendar day (1.xx to 365.xx)
    ...
    shr_orb_cosz = sin(lat)*sin(declin) - &
                   cos(lat)*cos(declin)*cos(jday*2._r8*pi + lon)
  ...
  subroutine shr_orb_decl (calday, eccen, mvelpp, lambm0, obliqr, delta, eccf)
    ...
    real(r8), intent(in)  :: eccen  ! Eccentricity
    real(r8), intent(in)  :: obliqr ! Earths obliquity in radians
    real(r8), intent(in)  :: lambm0 ! Mean longitude of perihelion at the vernal equinox (radians)
    real(r8), intent(in)  :: mvelpp ! Moving vernal equinox longitude of perihelion plus pi (radians)
    real(r8), intent(out) :: delta  ! Solar declination angle (radians)
    real(r8), intent(out) :: eccf   ! Earth-sun distance factor (i.e. (1/r)**2)
    !
    ! !LOCAL VARIABLES:
  ...
  subroutine shr_orb_params (iyear_AD, eccen, obliq, mvelp, obliqr, lambm0, mvelpp)
    ...
    ! !ARGUMENTS:
    implicit none
    integer, intent(in)     :: iyear_AD  ! Year to calculate orbit
    real(r8), intent(out)   :: eccen     ! Orbital eccentricity
    real(r8), intent(out)   :: obliq     ! Obliquity in degrees
    real(r8), intent(out)   :: mvelp     ! Moving vernal equinox longitude
    real(r8), intent(out)   :: obliqr    ! Earth's obliquity in radians
    real(r8), intent(out)   :: lambm0    ! Mean longitude of perihelion at vernal equinox (radians)
    real(r8), intent(out)   :: mvelpp    ! Moving vernal equinox longitude of perihelion plus pi (radians)
    !
    ! !LOCAL VARIABLES:
    !
    ...
    real(r8),parameter :: psecdeg = 1._r8/3600._r8 ! arc sec to deg conversion
